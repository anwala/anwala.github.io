...
def getdistances(rows,vec1):
  distancelist=[]
  vec2=[bicluster(rows[i], id=i) for i in range(len(rows))]
  while len(clust) > 1:
        lowestpair = (0, 1)
...
def knnestimate(rows,vec1,k=5):
  # Get sorted distances
  dlist=getdistances(rows,vec1)
  avg=0.0
...
def kcluster(rows, k, distance=pearson):
  ...
  # Create k randomly placed centroids
    clusters = [[(ranges[i][1] - ranges[i][0]) + ranges[i][0]
